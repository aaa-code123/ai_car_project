...
# 馬達啟動緩升步數（每步間隔 5ms，共 3 步：0→33→66→100%）
_RAMP_STEPS = 3
_RAMP_DELAY = 0.005
...
class MotorController:
    ...
    def _set_motors(self, a_fwd, b_fwd, c_fwd, d_fwd, speed=None):
        ...
        for step in range(1, _RAMP_STEPS + 1):
            ramp_spd = int(spd * step / _RAMP_STEPS)
            self.pwm.setDutycycle(self.CHANNELS['A_PWM'], ramp_spd)
            self.pwm.setDutycycle(self.CHANNELS['B_PWM'], ramp_spd)
            self.pwm.setDutycycle(self.CHANNELS['C_PWM'], ramp_spd)
            self.pwm.setDutycycle(self.CHANNELS['D_PWM'], ramp_spd)
            time.sleep(_RAMP_DELAY)
